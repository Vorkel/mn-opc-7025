...
plt.figure(figsize=(12, 8))
shap.summary_plot(
    shap_values, X_shap, feature_names=feature_names, plot_type="bar", show=False
)
...
for i, feature in enumerate(top_features):
    ...
    shap.dependence_plot(
        feature, shap_values, X_shap, feature_names=feature_names, show=False
    )
...
for idx in sample_indices:
    if idx < len(X_shap):
        ...
        try:
            ...
            plt.title(
                f"SHAP Local - Exemple {idx} (Probabilité défaut: {prediction[1]:.3f})"
            )
            plt.tight_layout()
            plt.savefig(
                f"reports/shap_local_example_{idx}.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
...
if len(top_features) >= 2:
    ...
    plt.tight_layout()
    plt.savefig(
        f"reports/shap_interaction_{feature1}_{feature2}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
shap_report = {
    ...
    "visualizations": (
        ["shap_global_importance.png", "shap_feature_importance.png"]
    ...
    "insights": [
        ...
        (
            "Top 5 features représentent"
            f" {importance_df.head(5)['importance'].sum() / importance_df['importance'].sum() * 100:.1f}%"
