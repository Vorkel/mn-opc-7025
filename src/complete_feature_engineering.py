...
def create_complete_feature_set(client_data):
    """
    Génère les 153 features complètes attendues par le modèle à partir des données de base du client.

    Cette fonction prend les données de base du formulaire Streamlit et génère toutes les features
    nécessaires pour faire une prédiction avec le modèle entraîné.
    ...
    encodings = {
        ...
        'AGE_GROUP': {
            'Young': 0, 'Adult': 1, 'Middle': 2, 'Senior': 3, 'Elder': 4
        },
        'EMPLOYMENT_GROUP': {
            'Unemployed': 0, 'New': 1, 'Short': 2, 'Medium': 3, 'Long': 4
        },
        'INCOME_GROUP': {
            'Low': 0, 'Medium': 1, 'High': 2, 'Very High': 3, 'Ultra High': 4
        },
        'CREDIT_GROUP': {
            'Small': 0, 'Medium': 1, 'Large': 2, 'Very Large': 3, 'Ultra Large': 4
        }
    }
    ...
    for col in df.columns:
        if df[col].dtype == 'object':
            ...
            if len(unique_values) <= 50:  # Eviter les mappings trop grands
                ...
                df[col] = df[col].apply(lambda x: hash(str(x)) % 1000 if x is not None else 0)
